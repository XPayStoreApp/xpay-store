...
import { Ban, CheckCircle2, Search, Wallet, X as XIcon } from "lucide-react";
...
export default function Users() {
  ...
  const load = () => {
    ...
    get<UserRow[]>(`/users${qs ? `?${qs}` : ""}`).then(setItems).catch(() => {});
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={load} className="bg-brand-600 text-white px-5 py-2 rounded-lg font-semibold hover:bg-brand-700">
          بحث
        </button>
      </div>
...
function BalanceModal({ user, onClose, onDone }: { user: UserRow; onClose: () => void; onDone: () => void }) {
